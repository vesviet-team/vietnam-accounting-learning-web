...
import { getProhibitionRule, getProhibitedAccounts133 } from '@/data/prohibited-accounts';
...
export const CoaExplorer: FC<CoaExplorerProps> = ({
  ...
  // Filter accounts with accent-insensitive search
  const filteredAccounts = useMemo(() => {
    const q = searchQuery.trim().toLowerCase();
    const qNoAccents = removeVietnameseAccents(q);

    if (showOnlyProhibited && currentRegime === 'CIRCULAR_133') {
      return getProhibitedAccounts133().filter((acc) => {
        if (selectedCategory !== 'ALL' && acc.category !== selectedCategory) return false;
        if (!q) return true;
        const code = acc.code.toLowerCase();
        const name = acc.nameVi.toLowerCase();
        const nameNoAccents = removeVietnameseAccents(name);
        const desc = acc.description.toLowerCase();
        const descNoAccents = removeVietnameseAccents(desc);
        return (
          code.includes(q) ||
          name.includes(q) ||
          nameNoAccents.includes(qNoAccents) ||
          desc.includes(q) ||
    ...
    const baseAccounts = getAccountsByRegime(currentRegime).filter((account) => {
    ...
    if (!q) return baseAccounts;

    return baseAccounts.filter((account) => {
      const code = account.code.toLowerCase();
      const name = account.nameVi.toLowerCase();
      const nameNoAccents = removeVietnameseAccents(name);
      const desc = account.description.toLowerCase();
      const descNoAccents = removeVietnameseAccents(desc);
      const sub = account.substituteIn133?.toLowerCase() || '';
      const subNoAccents = removeVietnameseAccents(sub);
      ...
      return (
        ...
        descNoAccents.includes(qNoAccents) ||
        sub.includes(q) ||
        subNoAccents.includes(qNoAccents)
      );
    });
      ...
      {filteredAccounts.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredAccounts.map((account) => {
            ...
            const isProhibited = account.isProhibitedIn133 || (currentRegime === 'CIRCULAR_133' && !!prohibition);
